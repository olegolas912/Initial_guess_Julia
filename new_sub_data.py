...
def build_variations(template_text: str, total: int,
                     a_min: int, a_max: int) -> List[Tuple[int, int, str]]:
    """
    Возвращает список (a, b, modified_text) для всех a ∈ [a_min, a_max] с b = total - a.
    Меняется только первая строка после TSTEP, остальные данные остаются неизменны.
    """
...
def process_folder(folder: Path, total_days: int, a_min: int, a_max: int, out_subdir: str) -> None:
    """Создаёт набор .DATA-файлов с TSTEP=(a,b), где a∈[a_min..a_max], a+b=total_days."""
    ...
    var_list = build_variations(text, total_days, a_min, a_max)
...
def main() -> None:
    ...
    ap.add_argument("--bhp-start", type=int, default=390, help="Начальное значение BHP (в имени папки)")
    ap.add_argument("--bhp-stop", type=int, default=399, help="Конечное значение BHP (в имени папки, включительно)")
    ap.add_argument("--total-days", type=int, default=400, help="Сумма первых двух чисел в TSTEP")
    ap.add_argument("--a-min", type=int, default=370, help="Минимальное значение первого числа (окно)")
    ap.add_argument("--a-max", type=int, default=390, help="Максимальное значение первого числа (окно)")
    ...
    for bhp in range(args.bhp_start, args.bhp_stop + 1):
        folder = base_dir / f"BHP_{bhp:03d}"
        if folder.is_dir():
            process_folder(folder, args.total_days, args.a_min, args.a_max, args.out_subdir)
        else:
